...
function Actions<T extends object>() {
  const [state, setState] = useState<state_type<T>>({modal_visibility: false, modal_type: null});
  const {status, filters, activated_filter, onSelectFilter, onRemoveFilter, searched_value, input_search, onSearchTerm, onClearTerm} =
    useContext((TableList.contextType)) as context_type<T>;

  const showModal = (type: modal_type) => setState({...state, modal_visibility: true, modal_type: type});
  ...
  return (
    <View style={[styles.h, SHADOW_SM_X, Style.bg__white, Style.flex__row,
        ...
        <View style={[Style.col__6, Style.h__100, Style.flex__row,
          ...
          <Conditional>
            ...
            <If condition={Boolean(filters?.length)}>
              <AppModal
                backdrop visibility={state.modal_visibility && state.modal_type === 'filter'} position={"bottom"}
                onClose={() => hideModal()}>
                <View style={[Style.w__100, {borderRadius: BORDER_RADIUS_SM},]}>
                  <View style={[Style.bg__white, Style.mt__auto]}>
                    <List>
                      <ListItem style={[Style.px__3, Style.mx__0]} key='all'
                                onPress={() => {
                                  onRemoveFilter();
                                  hideModal();
                                }}>
                        <Left><AppText>{translate('all')}</AppText></Left>
                        <Right>
                          <Icon type={'MaterialIcons'}
                                style={[activated_filter === null ? Style.text__info : null]}
                                name={`radio-button-${activated_filter === null ? '' : 'un'}checked`}/>
                        </Right>
                      </ListItem>
        ...
        <View style={[Style.col__6, Style.h__100, Style.flex__row,
          ...
          <Conditional>
            ...
            <If condition={Boolean(input_search)}>
              <AppModal
                backdrop visibility={state.modal_visibility && state.modal_type === 'search'} position={"bottom"}
                onClose={() => hideModal()}>
                <View style={[Style.w__100, {borderRadius: BORDER_RADIUS_SM},]}>
