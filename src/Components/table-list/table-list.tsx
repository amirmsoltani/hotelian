...
export type context_type<T> = props_type<T> &
  state_type<T> & {
...
class TableList<T> extends Component<
  props_type<T>,
  state_type<T>,
  context_type<T>
> {
  ...
  onSearchTerm = (term: string | undefined) => {
    const data = this.state.activated_filter
      ? this.searchTerm(
          term,
          this.selectFilter(this.state.activated_filter, this.props.data)
        )
  ...
  private searchTerm(term: string | undefined, data: T[]): T[] {
    if (
      ...
      return data.filter((item) => {
        ...
        if (el)
          return ((el as unknown) as string).indexOf(term.toLowerCase()) > -1;
        else return false;
      });
    }
    return data;
